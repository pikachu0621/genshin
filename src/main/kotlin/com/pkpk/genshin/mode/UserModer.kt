...
data class UserModer(
    ...
    @TableField(value = "is_bool_lock")
    var isBoolLock: Boolean = false,             // 是否拉黑   (ture = 1 = 锁定 / false = 0 = 未锁定 )

    @TableField(value = "up_time", updateStrategy = FieldStrategy.IGNORED)
    var upTime: String? = null,

    @TableField(value = "is_bool_ctoken")
    var isBoolCToken: Boolean = true,           // ctoken(cookie_token) 是否过期 (ture = 1 = 可用 / false = 0 = 不可用 )

    @TableField(value = "is_bool_stoken")
    var isBoolSToken: Boolean = true,           // stoken 是否过期 (ture = 1 = 可用 / false = 0 = 不可用 )

    @TableField(value = "is_bool_vcode")
    var isBoolVCode: Boolean = false,           //   是否触发验证码  (ture = 1 = 触发 / false = 0 = 未触发 )
    ...
    constructor(
        ...
        password: String? = null,
        addTime: String? = null,
        isBoolLock: Boolean = false,
        upTime: String? = null,
        isBoolCToken: Boolean = true,
        isBoolSToken: Boolean = true,
        isBoolVCode: Boolean = false,
    ) : this(
        id,
        uid,
        region,
